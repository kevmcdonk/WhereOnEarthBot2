...
2. Add your user password to TEAMSFX_API_BINGAPI_PASSWORD in `bot/.env.teamsfx.local`.

You can import the API client (an Axios instance) in another file and call bingAPI APIs and authentication is now handled for you automatically.

Here is an example for a GET request to "relative_path_of_target_api":
```
import { bingAPIClient } from "relative_path_to_this_file";

const response = await bingAPIClient.get("relative_path_of_target_api");
...
// your code will be: const response = await bingAPIClient.get("test");
...
*/
import { TeamsFx, createApiClient, BasicAuthProvider, ApiKeyProvider } from "@microsoft/teamsfx";

// Load application configuration
const teamsFx = new TeamsFx();
// Initialize a new axios instance to call bingAPI
const authProvider = new ApiKeyProvider(keyname, keyvalue, keylocation)
...
TEAMSFX_API_BINGAPI_ENDPOINT
TEAMSFX_API_BINGAPI_USERNAME
